...
Now supports placement subfolders under the spawn directory:
    spawn/floor/*.mcfunction
    spawn/wall/*.mcfunction
    spawn/ceiling/*.mcfunction

Validation rules for summon lines:
- No leading slashes
- Coordinates are ~ ~ ~
...
def validate_spawn_functions(spawn_function_dir):
    """Validate and fix all spawn function files recursively.

    Traverses placement subdirectories (floor, wall, ceiling) and fixes issues.

    Args:
        spawn_function_dir: Base spawn directory path

    Returns:
        dict: Summary of validation results
    """
    print(f'\nValidating spawn function files in: {spawn_function_dir} (recursive)')
    ...
    for root, _dirs, files in os.walk(spawn_function_dir):
        for filename in files:
            ...
            if not content.strip():
                ...
                print(f'  ⚠ {os.path.relpath(filepath, spawn_function_dir)}: Empty file')
                continue
            ...
            if modified:
                ...
                rel = os.path.relpath(filepath, spawn_function_dir)
                print(f'  ✓ {rel}: Fixed {len(file_issues)} issue(s)')
                for issue in file_issues:
                    print(f'    - {issue}')
            else:
                results['valid_files'] += 1
                rel = os.path.relpath(filepath, spawn_function_dir)
                print(f'  ✓ {rel}: Already valid')
